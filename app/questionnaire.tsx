...
export default function Questionnaire() {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			// Gerar userId como no Next.js
			const userId = `user_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`

			// Salvar no ProfileService para usar depois
			...
			Alert.alert(
				"Erro",
				error instanceof Error ? error.message : "Não foi possível processar o questionário. Tente novamente.",
				[{ text: "OK" }]
			)
		} finally {
			setIsLoading(false)
